...
@component('dashboard::components.inputs', $input_array = $formPresenter->setValue($input_setting, 'radio'))
    @foreach ($input_array['option'] as $key => $element)
        <div class="form-check form-check-inline">
            <input type="checkbox" class="form-check-input" id="{{ $input_array['input_name'] . $key }}"  name="{{ $input_array['input_name'] }}[]" value="{{ $key }}" {{ $input_array['input_value'] == $key ? ' checked' : '' }} {!! $input_array['attribute'] !!}>
