        ...
        @slot('page_title')
            @if ($blog_category_id)
                @if (isset($duplicate) && $duplicate)
                    @lang('dashboard::backend.複製')
                @else
                    @lang('base::blog.編輯部落格分類')
                @endif
            @else
                @lang('base::blog.新增部落格分類')
            @endif
        @endslot

        <div class="form-body">
            @slot('top_btn')
                <a class="btn btn-outline-deep-purple waves-effect d-xs-block" href="{{ url($uri . 'index?' . $base_service->getQueryString(true, true, ['blog_category_id'])) }}">
                    <i class="fa fa-fw fa-arrow-left"></i>@lang('base::blog.部落格分類')
                </a>
            @endslot
            @include('dashboard::backend-update-input', ['form_array' => $form_array, 'form_value' => $blog_category ?? ''])
        </div>
    @endcomponent
@endsection
