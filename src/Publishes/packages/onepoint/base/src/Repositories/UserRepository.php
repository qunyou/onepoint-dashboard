    ...
    public function setUpdate($id = 0, $datas = [])
    {
        ...
        $rule_array = [
            ...
            'username' => [
                'required',
                'max:255',
                Rule::unique($this->model->getTable())->ignore($id)->whereNull('deleted_at'),
            ],
            'email' => [
                'required',
                'max:255',
        ...
        $custom_name_array = [
            'realname' => __('dashbaord::auth.姓名'),
            'username' => __('dashbaord::auth.帳號'),
            'email' => __('dashbaord::auth.Email'),
            'password' => __('dashbaord::auth.密碼'),
        ];
        if (!$id) {
