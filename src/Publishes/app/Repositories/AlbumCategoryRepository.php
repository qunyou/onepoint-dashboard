...
use App\Entities\AlbumCategory;
...
class AlbumCategoryRepository extends BaseRepository
{
    ...
    {
        parent::__construct(new AlbumCategory);
    }
    ...
    {
        $query = $this->permissions()->withCount('album');
    ...
    public function setUpdate($id = 0, $datas = [])
    {
        ...
        $rule_array = [
            'category_name' => [
                'required',
                'max:255',
                Rule::unique('album_categories')->ignore($id)->whereNull('deleted_at'),
            ]
        ];
        $custom_name_array = [
