...
class NewsCategoryRepository extends BaseRepository
{
    ...
    {
        parent::__construct(new NewsCategory);
    }
    ...
    {
        $query = $this->permissions()->withCount('news');
    ...
    {
        $query = $this->permissions()->with('news');
    ...
    public function setUpdate($id = 0, $datas = [])
    {
        ...
        $rule_array = [
            'category_name' => [
                'required',
                'max:255',
                Rule::unique('news_categories')->ignore($id)->whereNull('deleted_at'),
            ]
        ];
        $custom_name_array = [
